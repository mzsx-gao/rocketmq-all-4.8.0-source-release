...
public class ProducerDetails {
    public static void main(String[] args) throws Exception{
        ...
        //消息重试时选择另外一个Broker（消息没有存储成功是否发送到另外一个broker），默认为false
        producer.setRetryAnotherBrokerWhenNotStoreOK(false);
        //允许发送的最大消息长度，默认为4M
        producer.setMaxMessageSize(1024 * 1024 * 4);

        // 设置NameServer的地址
        producer.setNamesrvAddr("47.100.11.132:9876");//106.55.246.66
        // 启动Producer实例
        producer.start();
        //0 查找该主题下所有消息队列
        ...
        for (int i = 0; i < 10; i++) {
            ...
            //2.2同步超时发送消息(属性设置：sendMsgTimeout 发送消息默认超时时间，默认3s (3000ms) )
            SendResult sendResult1 = producer.send(msg,1000*3);
            //2.3指定队列同步发送消息(使用select方法)
            ...
            producer.send(msg,MessageQueue.get(0), new SendCallback() {
                @Override
                public void onSuccess(SendResult sendResult) {
                    System.out.printf("%-10d OK %s %n", index, sendResult.getMsgId());
                }
